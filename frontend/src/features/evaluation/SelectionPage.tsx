...
const SelectionPage = () => {
    ...
    // Listas dinâmicas baseadas nos alunos cadastrados
    ...
    const getDiagnosisColor = (diagnosis: string | undefined) => {
        ...
        if (d.includes('tea') || d.includes('autismo')) return 'rgba(168, 85, 247, 0.15)'; // Roxo
        if (d.includes('tdah')) return 'rgba(249, 115, 22, 0.15)'; // Laranja
        if (d.includes('dislexia')) return 'rgba(59, 130, 246, 0.15)'; // Azul
        if (d.includes('nenhum') || d === '') return 'transparent';
        return 'rgba(234, 179, 8, 0.12)'; // Amarelo/Outros
    };
    ...
    useEffect(() => {
        const fetchAlunos = async () => {
            setLoading(true);
            try {
                // Buscamos todos para filtrar no client-side para melhor UX com múltiplos filtros
                        ...
                        <tbody>
                            {filteredAlunos.map(aluno => (
                                <tr
                                    ...
                                    onClick={() => navigate(`/leitura/${aluno.id}`)}
                                >
                                    ...
                                    <td style={{ padding: '1.25rem 2rem' }}>
                                        <span style={{
                                            fontSize: '0.8rem',
                                            fontWeight: 600,
                                            color: aluno.diagnostico && aluno.diagnostico.toLowerCase() !== 'nenhum' ? 'white' : 'var(--text-muted)'
                                        }}>
